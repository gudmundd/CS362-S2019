...
int main (int argc, char** argv) {
	...
	for (game_state_pass = 1; game_state_pass <= num_game_state_tests; game_state_pass++)
	{
		...
		// REQUIREMENT 
		// Invalid hand because player doesn't actually have Adventurer to play
		initializeGame(4, k, game_state_pass, &G);
		memcpy(&G_orig, &G, sizeof(struct gameState));
		G.hand[current_player][G.handCount[current_player]++] = mine;
		int ret;
		// Execute the card function
		ret = cardEffect(adventurer, 0, 0, 0, &G, G.hand[current_player][G.handCount[current_player]-1], 0);
		if (ret == 0)
			printf("FAIL: Invalid hand - player didn't have Adventurer\n");
		else
			printf("PASSED: Invalid hand - player didn't have Adventurer\n");	
		...
		// Execute the card function
		ret = cardEffect(adventurer, 0, 0, 0, &G, G.hand[invalid_current_player][G.handCount[invalid_current_player]-1], 0);
		if (ret == 0)
		...
		// Execute the card function
		ret = cardEffect(adventurer, 0, 0, 0, &G, invalid_hand_pos, 0);
		if (ret == 0)
		...
		// REQUIREMENT
		// Invalid choice1, choice2, and choice3 - not required for Adventurer, so should be zero and not be ignored by cardEffect
		initializeGame(4, k, game_state_pass, &G);
		memcpy(&G_orig, &G, sizeof(struct gameState));
		G.hand[current_player][G.handCount[current_player]++] = adventurer;
		...
		// Execute the card function
		ret = cardEffect(adventurer, invalid_choice_1, invalid_choice_2, invalid_choice_3, &G, G.hand[current_player][G.handCount[current_player]-1], 0);
		if (ret == 0)
		...
		// REQUIREMENT
		// Invalid bonus - not required for Adventurer, so should be zero and not be ignored by cardEffect
		initializeGame(4, k, 2, &G);
		memcpy(&G_orig, &G, sizeof(struct gameState));
		G.hand[current_player][G.handCount[current_player]++] = adventurer;
		int invalid_bonus = 99;
		// Execute the card function
		ret = cardEffect(adventurer, 0, 0, 0, &G, G.hand[current_player][G.handCount[current_player]-1], &invalid_bonus);
		if (ret == 0)
	...
	G.hand[current_player][G.handCount[current_player]++] = adventurer;
	// Execute the card function
	ret = cardEffect(adventurer, 0, 0, 0, &Uninitialized_G, G.hand[current_player][G.handCount[current_player]-1], 0);
