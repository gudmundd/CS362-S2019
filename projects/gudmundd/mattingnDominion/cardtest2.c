/*
	...
	Card Tested:		council_room in dominion.c
...
char CARD_UNDER_TEST[] = "council_room";	// string for printing the unit test name
...
int main (int argc, char** argv) {
	...
	int errr_2 = 0;
	int errr_3 = 0;
	int k[10] = {adventurer, gardens, embargo, village, minion, mine, council_room,  // change cutpurse to council_room
		   sea_hag, tribute, smithy};

	printf ("Starting Card Test 2 - %s.\n", CARD_UNDER_TEST);
	...
	for (game_state_pass = 1; game_state_pass <= num_game_state_tests; game_state_pass++)
	{
		...
		// Add the council_room card to the current player's hand
		G.hand[current_player][G.handCount[current_player]++] = council_room;
		// Execute the card function
		//cardEffect(council_room, 0, 0, 0, &G, G.hand[current_player][G.handCount[current_player]-1], 0);  // REQUIRED CHANGE for Assignment 5
		playCouncilRoom(&G, G.hand[current_player][G.handCount[current_player]-1]);						    // REQUIRED CHANGE for Assignment 5
		
		// REQUIREMENT
		// Current player draws 4 cards, and discards 1, after already receiving council_room, so net add is 4
		for (i=0; i<G.numPlayers; i++){
			if (i==0){
				if ((G.handCount[i] - 4 != G_orig.handCount[i]) || FORCE_FAIL){
					printf("FAIL: Current player handCount Error: G.handCount[%d]=%d, G_orig.handCount[%d]=%d\n", i, G.handCount[i], i, G_orig.handCount[i]);
					errr_1 = 1;
				}
			}
		// REQUIREMENT
		// Other players each add 1 card
			if (i > 0){
		...
		if (errr_1 == 0)
			printf("PASS - Current player's handCount increases by 4 (+1 council_room, +4 drawn, -1 discard = 4)\n");
		if (errr_2 == 0)
			printf("PASS - Other player's each add one card\n");
			
		// REQUIREMENT
		// Current player's number of buys increases by 1
		if(G.numBuys != G_orig.numBuys + 1)
			printf("FAIL: Current player's number of buys should increase by 1\n");
		else
			printf("PASS - Current player's number of buys increased by 1\n");
		...
		if (errr_1 == 0)
			printf("PASS - Current player's deck reduces by 4\n");
		if (errr_2 == 0)
			printf("PASS - Other players' decks reduce by 1\n");
