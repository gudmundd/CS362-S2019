/*
	...
	Card Tested:	Council Room in dominion.c
	
	Makefile:		Type "make randomtestresults" to compile, run, anc generate coverage results 
					for all three tests, and to pipe the results to randomtestresults.out
...
void rand_test_council_room() 
{
	...
	char CARD_UNDER_TEST[] = "Council Room";	// string for printing the card under test
	int FORCE_FAIL = 0;
	...
	for (i = 0; i < num_random_tests; i++)
	{
		...
		// Guarantee the current player had an council room card
		// by randomly seeding one
		current_player_handCount = PRE.handCount[current_player];
		z = rand() % current_player_handCount;
		PRE.hand[current_player][z] = council_room;	
		
		// BEGIN CARD TESTS
		failed_game_state_flag = 0;
		
		// REQUIREMENT
		// Verify current player begins with 1 buy
		if (PRE.numBuys != 1) {
			printf("FAIL INIT - Iteration %d: game should start with number of buys = 1\n", i);
			num_tests_failed++;
		...
		if (cardEffect(council_room, 0, 0, 0, &POST, POST.handCount[z], 0) || FORCE_FAIL) {
		...
		// REQUIREMENT
		// Current player's number of buys should increase by 1
		if(POST.numBuys - 1 != PRE.numBuys || FORCE_FAIL) {
			printf("FAIL REQ 2 - Iteration %d: Current player's number of buys should increase by 1\n", i);
			num_tests_failed++;
			failed_game_state_flag = 1;
		}
		else {
		...
		// REQUIREMENT
		// Current player draws 4 cards, and discards 1, so net in the current player's hand is 3 cards
		if ((POST.handCount[current_player] -3 != PRE.handCount[current_player]) || FORCE_FAIL){
		...
		// REQUIREMENT
		// Other players draw a card, so handCount increases by 1
		for (x = 1; x < POST.numPlayers; x++){ 
			if ((POST.handCount[x] - 1 != PRE.handCount[x]) || FORCE_FAIL) {
				printf("FAIL REQ 4 - Iteration %d: Other player[%d] handCount should increase by 1\n", i, x);
				num_tests_failed++;
				failed_game_state_flag = 1;
			}
			else {
		...
		// REQUIREMENT
		// Other players draw a card, so deckCount decreases by 1
		for (x = 1; x < POST.numPlayers; x++){ 
			if ((POST.deckCount[x] + 1 != PRE.deckCount[x]) || FORCE_FAIL) {
				printf("FAIL REQ 5 - Iteration %d: Other player[%d] deckCount should decrease by 1\n", i, x);
				num_tests_failed++;
				failed_game_state_flag = 1;
			}
			else {
		...
		// REQUIREMENT
		// Current player's deck reduces by 4
		if (POST.deckCount[current_player] + 4 != PRE.deckCount[current_player] || FORCE_FAIL) {
			printf("FAIL REQ 6 - Iteration %d: Current player deckCount should decrease by 4\n", i);
			num_tests_failed++;
			failed_game_state_flag = 1;
		}
		else {
		...
		if(PRE.coins != POST.coins || FORCE_FAIL)
			rest_of_game_state_err = 1;		
		if(PRE.numBuys != POST.numActions || FORCE_FAIL)
			rest_of_game_state_err = 1;		
