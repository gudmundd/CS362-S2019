/*
	...
	Makefile:			The following lines are added to the makefile provided in CS362
							...
							unittest1: unittest1.c dominion.o rngs.o 
								gcc -o ct1 unittest1.c -g dominion.o rngs.o $(CFLAGS)
...
void print_game_state(struct gameState G, struct gameState G_orig);  	// for debug only, not used in unit testing
...
int main (int argc, char** argv) {
	...
	for (game_state_pass = 1; game_state_pass <= num_game_state_tests; game_state_pass++)
	{
		...
		// REQUIREMENT
		// Current player draws 3 cards, and discards 1, after already receiving Smithy, so net add is 3
		G.hand[current_player][G.handCount[current_player]++] = smithy;						
		// Execute the card function
		//cardEffect(smithy, 0, 0, 0, &G, G.hand[current_player][G.handCount[current_player]-1], 0);   // REQUIRED CHANGE for Assignment 5
		playSmithy(&G, G.hand[current_player][G.handCount[current_player]-1]);						   // REQUIRED CHANGE for Assignment 5
		for (i=0; i<G.numPlayers; i++){
			if (i==0){
				if ((G.handCount[i] - 3 != G_orig.handCount[i]) || FORCE_FAIL){
		...
		if (errr_1 == 0)
			printf("PASS - Current player's handCount increases by 3 (+1 smithy, +3 drawn, -1 discard = 3)\n");
		if (errr_2 == 0)
			printf("PASS - Other players' handCounts remain unchanged\n");

		// REQUIREMENT
		// Current player's deck reduces by 3	
		errr_1 = errr_2 = 0;
		for (i=0; i<G.numPlayers; i++){
			if (i==0){
				if (G.deckCount[i] + 3 != G_orig.deckCount[i]){
		...
		// The following tests validate various remaining state variables are unchanged
		errr_1 = 0;
